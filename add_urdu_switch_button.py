...
linking to the corresponding Urdu translation.
"""
import os
import json
import re
...
def setup_logging():
    ...
    logging.basicConfig(
        ...
        handlers=[
            logging.FileHandler('switch_to_urdu.log'),
...
def create_switch_button(urdu_path: str) -> str:
    """Create the switch to Urdu button HTML."""
    # Create the button with inline JavaScript to switch to Urdu
    button_html = f'<div class="urdu-switch-button-container" style="margin-bottom: 20px; padding: 10px; background-color: #f0f8ff; border: 1px solid #b3d9ff; border-radius: 4px;">\n'
    button_html += f'  <button class="urdu-switch-button" onclick="switchToUrdu(\'{urdu_path}\')" style="background-color: #4CAF50; color: white; padding: 10px 16px; text-align: center; text-decoration: none; display: inline-block; font-size: 16px; margin: 4px 2px; cursor: pointer; border: none; border-radius: 4px;">\n'
    button_html += f'    Urdu mein dekhein\n'
    button_html += f'  </button>\n'
    button_html += f'</div>\n\n'
    return button_html


def create_javascript_function() -> str:
    """Create the supporting JavaScript function."""
    js_code = '<script>\n'
    js_code += '  function switchToUrdu(path) {\n'
    js_code += '    // Check if the Urdu file exists before redirecting\n'
    js_code += '    fetch(path, { method: "HEAD" })\n'
    js_code += '      .then(response => {\n'
    js_code += '        if (response.ok) {\n'
    js_code += '          window.location.href = path;\n'
    js_code += '        } else {\n'
    js_code += '          alert("Urdu translation not available yet. Please check back later.");\n'
    js_code += '        }\n'
    js_code += '      })\n'
    js_code += '      .catch(error => {\n'
    js_code += '        alert("Urdu translation not available yet. Please check back later.");\n'
...
def add_switch_button_to_content(
    frontmatter_data: Dict,
    content: str,
    urdu_path: str
) -> str:
    """Add the switch to Urdu button to the content."""
    # Create the button and JavaScript
    button_html = create_switch_button(urdu_path)
    js_code = create_javascript_function()

    # Combine button and JS with the original content
    updated_content = button_html + js_code + content
...
def main():
    ...
    logger.info("Starting to add 'Switch to Urdu' buttons to chapters")
    ...
    for i, file_path in enumerate(markdown_files, 1):
        ...
        try:
            ...
            # Add the switch button to the content
            updated_content = add_switch_button_to_content(frontmatter_data, content, urdu_path)
    ...
    # Show summary
    print("\n" + "="*50)
    print("SWITCH TO URDU BUTTONS SUMMARY")
    print("="*50)
